...
impl Library {
    ...
    pub fn fill_stock(&mut self, book: Book, amount: u32) {
        let stock = self.stock.entry(book).or_default();
        stock.log.push(Transaction::Fill(amount));
        stock.overall += amount;
    }
    ...
    pub fn lend(&mut self, book: &Book) -> Result<(), LibraryError> {
        self.stock
            ...
            .map(|stock| {
                stock.lent += 1;
                stock.log.push(Transaction::Lend);
            })
    ...
    pub fn take_back(&mut self, book: &Book) -> Result<(), LibraryError> {
        self.stock
            ...
            .map(|stock| {
                stock.lent -= 1;
                stock.log.push(Transaction::TakeBack);
            })
            .ok_or(LibraryError::InvalidOperation)
    }
    ...
    /// Get transaction log of a book
    pub fn get_transaction_log(&self, book: &Book) -> Option<&[Transaction]> {
        self.stock.get(book).map(|stock| stock.log.as_slice())
    }
...
mod tests {
    ...
    pub fn lend_take_back() {
        ...
        assert_eq!(
            library.get_transaction_log(&book1),
            Some(
                [
                    Transaction::Fill(2),
                    Transaction::Lend,
        ...
        assert_eq!(library.get_transaction_log(&book2), None);
    }
}
